...
import {
	TypePurchases,
	TypePurchasesProps,
} from "../../6_entitis/Purchases/PurchasesEntiti.ts"
			...
			const fetchFilms = async () => {
				const filmData = await Promise.all(
					Userpurchases.map(async (purchase) => {
						const response = await fetch(
							`http://localhost:5000/api/Film/${purchase.filmid}`
						)
						if (response.ok) {
	...
	useEffect(() => {
		...
		if (film.length && Userpurchases.length) {
			const updatedPurchasesProps = Userpurchases.map((purchase) => {
				const matchedFilm = film.find(
					(f) => Number(f.id) === Number(purchase.filmid)
				)
				if (matchedFilm) {
					return {
						...purchase,
