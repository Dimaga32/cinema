...
export default function PurchasesContent(
	...
	const totalCost: number = purchases.reduce((a: number, b: TypePurchasesProps) =>{
		if(b.status=="inProcess")
			return a + b.cost;
		else return a
	}, 0);
	...
	return (
		<div className={classes.container}>
			...
			<ul className={classes.list}>
				{sortedPurchases.map((purchase: TypePurchasesProps, index: number): ReactNode => {
						...
						return (
							<React.Fragment key={purchase.id}>
								<li className={classes.item + ` ` + classes[purchase.status]}>
									...
									<img src={purchase.image} alt={purchase.name} className={classes.image} />
									<div className={classes.info}>
										<h3 className={classes.name}>{purchase.name}</h3>
										<p className={classes.description}>{purchase.description}</p>
										<p className={classes.date}>{dataToStringer(purchase.data)}</p>
										<p className={classes.date}>Cost: {purchase.cost}</p>
									</div>
								</li>
								{index === lastInProcessIndex && (
									<div className={classes.totalCost + ` text-center`}>
										<h3>Total Cost: {totalCost}</h3>
										<button className={classes.Mybtn}>Оплатить</button>
									</div>
								)}
							</React.Fragment>
						);
